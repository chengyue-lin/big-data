...
object StreamWeather {
  ...
  val table = hbaseConnection.getTable(TableName.valueOf("chengyuel_final_weather_report"))
  ...
  def main(args: Array[String]) {
    ...
    val topicsSet = Set("chengyuel_final_weather")
    // Create direct kafka stream with brokers and topics
    ...
    val batchStats = reports.map(wr => {
      val put = new Put(Bytes.toBytes(wr.year+wr.month + wr.day))
